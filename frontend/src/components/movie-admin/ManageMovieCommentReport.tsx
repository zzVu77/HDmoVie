...
import { MovieCommentReportType } from '@/types'
import { columns } from './movie-comment-report-columns'
import reportService from '@/services/reportService'
import { toast } from 'sonner'

export function ManageMovieCommentReport() {
  const [data, setData] = React.useState<MovieCommentReportType[]>([])
  ...
  const fetchReports = React.useCallback(async () => {
    setLoading(true)
    try {
      const response = await reportService.getMovieCommentReports()
      const reports: MovieCommentReportType[] = response.map((report: any) => ({
        id: report.id,
        reporter: {
        ...
        movieId: report.comment.movie.id,
        title: report.comment.movie.title,
      }))
