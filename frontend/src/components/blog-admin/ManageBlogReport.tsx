...
import { columns } from './blog-report-columns'
import reportService from '@/services/reportService'
import { BlogReportType } from '@/types'
import { toast } from 'sonner'
export function ManageBlogReport() {
  const [data, setData] = React.useState<BlogReportType[]>([])
  ...
  const fetchReports = React.useCallback(async () => {
    setLoading(true)
    try {
      const response = await reportService.getBlogReports()
      // Transform the raw data to BlogCommentReportType
      const reports: BlogReportType[] = response.map((report: any) => ({
        id: report.id,
        reporter: {
