...
interface BlogPostComponentProps {
  ...
  isFirst?: boolean; // New prop for first post
  isLast?: boolean; // New prop for last post
}
...
const BlogPost = ({
  ...
  return (
    <Card
      className={cn(
        'overflow-hidden bg-zinc-900 border border-zinc-700 hover:shadow-md transition-shadow duration-300 m-0', // Added border
        isFirst && 'rounded-t-md rounded-b-none', // Round top for first post
        isLast && 'rounded-b-md rounded-t-none', // Round bottom for last post
        !isFirst && !isLast && 'rounded-none', // No rounding for middle posts
        className
      )}
    >
      ...
      <CardFooter
        className={cn(
          ...
          showFooterBorder ? 'border-t border-zinc-700' : 'border-none' // Updated footer border
        )}
      >
