...
// Middleware to check for RegisteredUser role
export const isRegisteredUser = (req: Request, res: Response, next: NextFunction): void => {
  if (res.locals.user && res.locals.user.role === 'RegisteredUser') {
    next()
  } else {
    res.status(403).json({ message: 'Access denied: RegisteredUser role required' })
  }
}
